...
use crate::geom::approxeq::ApproxEq;
use crate::geom::approxord::{max, min};
use crate::geom::length::Length;
use crate::geom::num::*;
use crate::geom::scale::Scale;
use crate::geom::size::{Size2D, Size3D};
use crate::geom::vector::{vec2, vec3, Vector2D, Vector3D};
...
impl<T: Copy, U> Point2D<T, U> {
    ...
    ///
    /// ```rust
    /// # use surfman::geom::{Point2D, point2};
    /// enum Mm {}
    ///
...
impl<T: PartialOrd, U> Point2D<T, U> {
    /// Return minimum point between this and another
    #[inline]
    pub fn min(self, other: Self) -> Self {
        point2(min(self.x, other.x), min(self.y, other.y))
    }

    /// Return maximum point between this and another
    #[inline]
...
impl<T: Float + Sub<T, Output = T>, U> Point2D<T, U> {
    /// Get the distance between this and the other.
    #[inline]
...
impl<T: Copy, U> Point3D<T, U> {
    ...
    ///
    /// ```rust
    /// # use surfman::geom::{Point3D, point3};
    /// enum Mm {}
    ///
    ...
    /// Go from Point3D to an array of length 4.
    #[inline]
    ...
    /// Go from Point3D to a tuple of length 4.
    #[inline]
    ...
    /// ```rust
    /// use surfman::geom::point3;
    /// use surfman::geom::default::Point3D;
    ///
...
impl<T: PartialOrd, U> Point3D<T, U> {
    /// Returns the minimum point between this and the other.
    #[inline]
    pub fn min(self, other: Self) -> Self {
        point3(
            min(self.x, other.x),
    ...
    /// Returns the maximum point between this and the other.
    #[inline]
    pub fn max(self, other: Self) -> Self {
        point3(
            max(self.x, other.x),
...
impl<T: Float + Sub<T, Output = T>, U> Point3D<T, U> {
    /// Find the distance from this to the other point.
    #[inline]
    pub fn distance_to(self, other: Self) -> T {
        (self - other).length()
    }
...
#[cfg(test)]
mod point2d {
    use crate::geom::default::Point2D;
    ...
    mod ops {
        use crate::geom::default::Point2D;
        use crate::geom::scale::Scale;
        use crate::geom::{size::size2, vector::vec2, vector::Vector2D};

        pub enum Mm {}
        pub enum Cm {}

        pub type Point2DMm<T> = crate::geom::point::Point2D<T, Mm>;
        pub type Point2DCm<T> = crate::geom::point::Point2D<T, Cm>;
...
#[cfg(test)]
mod point3d {
    use crate::geom::default;
    use crate::geom::default::Point3D;
    use crate::geom::point::{point2, point3};
    #[cfg(feature = "mint")]
    use mint;
    ...
    mod ops {
        use crate::geom::default::Point3D;
        use crate::geom::scale::Scale;
        use crate::geom::{size::size3, vector::vec3, vector::Vector3D};

        pub enum Mm {}
        pub enum Cm {}

        pub type Point3DMm<T> = crate::geom::point::Point3D<T, Mm>;
        pub type Point3DCm<T> = crate::geom::point::Point3D<T, Cm>;
