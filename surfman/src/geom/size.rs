...
use crate::geom::vector::{vec2, vec3, BoolVector2D, BoolVector3D, Vector2D, Vector3D};
#[cfg(feature = "mint")]
use mint;
...
impl<T: Copy, U> Size3D<T, U> {
    ...
    /// ```rust
    /// use surfman::geom::size3;
    /// use surfman::geom::default::Size3D;
    ///
...
mod size3d {
    mod ops {
        use crate::geom::default::Size3D;
        use crate::geom::scale::Scale;

        pub enum Mm {}
        pub enum Cm {}

        pub type Size3DMm<T> = crate::geom::size::Size3D<T, Mm>;
        pub type Size3DCm<T> = crate::geom::size::Size3D<T, Cm>;
