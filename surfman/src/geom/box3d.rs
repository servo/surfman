...
use crate::geom::num::*;
use crate::geom::point::{point3, Point3D};
use crate::geom::scale::Scale;
use crate::geom::size::Size3D;
    ...
    T: PartialOrd,
{
    ...
    /// Do this and the other box intersect?
    #[inline]
...
where
    T: Copy + PartialOrd,
{
    /// Returns the option'd non-empty box.
    #[inline]
    ...
    /// Returns the unchecked intersection.
...
#[cfg(test)]
mod tests {
    use crate::geom::default::{Box3D, Point3D};
    use crate::geom::{point::point3, size::size3, vector::vec3};
    ...
    fn test_nan_empty_or_negative() {
        use std::f32::NAN;
        assert!(Box3D {
            min: point3(NAN, 2.0, 1.0),
            max: point3(1.0, 3.0, 5.0)
        }
        .is_empty());
        assert!(Box3D {
            min: point3(0.0, NAN, 1.0),
            max: point3(1.0, 2.0, 5.0)
        }
        .is_empty());
        assert!(Box3D {
            min: point3(1.0, -2.0, NAN),
            max: point3(3.0, 2.0, 5.0)
        }
        .is_empty());
        assert!(Box3D {
            min: point3(1.0, -2.0, 1.0),
            max: point3(NAN, 2.0, 5.0)
        }
        .is_empty());
        assert!(Box3D {
            min: point3(1.0, -2.0, 1.0),
            max: point3(0.0, NAN, 5.0)
        }
        .is_empty());
        assert!(Box3D {
            min: point3(1.0, -2.0, 1.0),
            max: point3(0.0, 1.0, NAN)
        }
        .is_empty());
    }
}
