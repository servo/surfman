...
static TRI_VERTICES: [Vertex; 3] = [
    Vertex { x:  0.0, y:  0.5, r: 255, g: 0,   b: 0,   a: 255 },
    Vertex { x:  0.5, y: -0.5, r: 0,   g: 255, b: 0,   a: 255 },
    Vertex { x: -0.5, y: -0.5, r: 0,   g: 0,   b: 255, a: 255 },
];

static APP_NAME: &'static str = "surfman offscreen example";

fn main() {
    let matches = App::new(APP_NAME).arg(Arg::with_name("hardware").short("H")
                                                                   .long("hardware")
                                                                   .help("Use hardware rendering"))
                                    .arg(Arg::with_name("software").short("s")
                                                                   .long("software")
    ...
    unsafe {
        ...
        gl::BindVertexArray(tri_vertex_array.object); ck();
        gl::UseProgram(tri_vertex_array.tri_program.program.object); ck();
        gl::DrawArrays(gl::TRIANGLES, 0, 3); ck();
        gl::Flush(); ck();
...
impl TriVertexArray {
    fn new(gl_api: GLApi, gl_texture_target: GLenum) -> TriVertexArray {
        ...
        unsafe {
            ...
            gl::VertexAttribPointer(tri_program.position_attribute as GLuint,
                                    2,
                                    gl::FLOAT,
                                    gl::FALSE,
                                    12,
                                    0 as *const GLvoid); ck();
            gl::VertexAttribPointer(tri_program.color_attribute as GLuint,
                                    4,
                                    gl::UNSIGNED_BYTE,
                                    gl::TRUE,
                                    12,
                                    8 as *const GLvoid); ck();
            gl::EnableVertexAttribArray(tri_program.position_attribute as GLuint); ck();
            gl::EnableVertexAttribArray(tri_program.color_attribute as GLuint); ck();

            TriVertexArray { object: vertex_array, tri_program, vertex_buffer }
        }
    }
}
...
impl TriProgram {
    fn new(gl_api: GLApi, gl_texture_target: GLenum) -> TriProgram {
        ...
        unsafe {
            ...
            let color_attribute =
                gl::GetAttribLocation(program.object, "aColor\0".as_ptr() as *const GLchar); ck();
            TriProgram { program, position_attribute, color_attribute }
        }
    }
}
